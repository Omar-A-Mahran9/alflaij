...
$(document).ready(() => {
    ...
    colorsSp.change(function () {
        ...
        currentlySelected.forEach((colorId) => {
...
let openImagesModal = (colorId) => {
    ...
    let selectedImages = carImageSorted.filter(item => item.color_id == colorId).sort((a, b) => a.sort - b.sort);
    $("#images-container").empty();
    
    if (selectedImages.length > 0) {
        $("#no-images-text").addClass("d-none");
        selectedImages.forEach(item => {
            $("#images-container").append(createImageContainer(item.image));
        });
    } else {
...
let createImageContainer = (image) => {
    let imagePath = getImagePathFromDirectory(image, "Cars");
    return `
        <div class="col-md-3 col-12 my-4 text-center">
            <div class="image-input image-input-outline">
                <div class="image-input-wrapper w-100px h-100px" style="background-image: url('${imagePath}'); background-size: contain;"></div>
            </div>
        </div>`;
};
...
let initializeColorsSp = () => {
    ...
    colorsSp.val(tempArr);
    tempArr.forEach(() => colorsSp.trigger('change'));
};
